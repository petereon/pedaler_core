...
This document provides comprehensive instructions for integrating the Pedaler Core circuit simulator into web applications via WebAssembly. It covers TypeScript and Elm integration patterns with Web Audio API.
...
6. [Elm Integration](#elm-integration)
7. [AudioWorklet Patterns](#audioworklet-patterns)
...
Elm cannot directly call WASM functions, so integration uses **ports** to communicate with JavaScript.

### Architecture Overview

```
┌─────────────┐     Ports      ┌──────────────┐     WASM     ┌─────────────┐
│     Elm     │ ◄────────────► │  JavaScript  │ ◄──────────► │   Pedaler   │
│   (Model)   │                │   (Bridge)   │              │    Core     │
└─────────────┘                └──────────────┘              └─────────────┘
```
...
```elm
-- src/Audio/Pedaler.elm
port module Audio.Pedaler exposing
    ( Circuit
    , Config
    , Msg(..)
    , SimulatorState(..)
...
defaultConfig : Circuit -> Config
defaultConfig circuit =
    { circuit = circuit
    , sampleRate = 48000
    , maxIterations = 50
    , tolerance = 1.0e-4
...
port pedalerConnect : Encode.Value -> Cmd msg


port pedalerDisconnect : () -> Cmd msg


port pedalerUpdateCircuit : String -> Cmd msg
...
port pedalerStateChanged : (Decode.Value -> msg) -> Sub msg


port pedalerError : (String -> msg) -> Sub msg
...
connect config =
    pedalerConnect (encodeConfig config)
...
updateCircuit : Circuit -> Cmd msg
updateCircuit circuit =
    pedalerUpdateCircuit circuit
...
subscriptions toMsg =
    Sub.batch
        [ pedalerStateChanged (decodeState >> toMsg)
        , pedalerError (ErrorOccurred >> toMsg)
        ]
...
encodeConfig config =
    Encode.object
        [ ( "circuit", Encode.string config.circuit )
        , ( "sampleRate", Encode.float config.sampleRate )
        , ( "maxIterations", Encode.int config.maxIterations )
        , ( "tolerance", Encode.float config.tolerance )
        ]


decodeState : Decode.Value -> Msg
decodeState value =
    case Decode.decodeValue stateDecoder value of
        ...
        Err err ->
            ErrorOccurred (Decode.errorToString err)


stateDecoder : Decode.Decoder SimulatorState
stateDecoder =
        ...
        |> Decode.andThen
            (\state ->
                case state of
                    ...
                    _ ->
                        Decode.fail ("Unknown state: " ++ state)
            )
```
...
import Audio.Pedaler as Pedaler
import Browser
...
type alias Model =
    { circuit : String
    , simulatorState : Pedaler.SimulatorState
    , audioEnabled : Bool
    }
...
# Tube screamer style soft clipping
V_IN in 0 DC 0
R1 in clip 4.7k
D1 clip 0 1N4148
D2 0 clip 1N4148
...
init _ =
    ( { circuit = initialCircuit
      , simulatorState = Pedaler.Disconnected
      , audioEnabled = False
      }
    , Cmd.none
    )
    ...
    case msg of
        ToggleAudio ->
            if model.audioEnabled then
                ...
                ( { model
                    | audioEnabled = True
                    , simulatorState = Pedaler.Connecting
                  }
                , Pedaler.connect (Pedaler.defaultConfig model.circuit)
                )

        CircuitChanged newCircuit ->
            ( { model | circuit = newCircuit }
            , if model.audioEnabled then
                Pedaler.updateCircuit newCircuit

              else
                Cmd.none
            )

        PedalerMsg pedalerMsg ->
            case pedalerMsg of
                Pedaler.StateChanged state ->
                    ( { model | simulatorState = state }, Cmd.none )

                Pedaler.ErrorOccurred error ->
                    ( { model | simulatorState = Pedaler.Error error }, Cmd.none )
...
view : Model -> Html Msg
view model =
    div [ class "pedaler-app" ]
        [ h1 [] [ text "Pedaler Circuit Simulator" ]
        , viewStatus model.simulatorState
...
viewStatus : Pedaler.SimulatorState -> Html msg
viewStatus state =
    let
        ( statusClass, statusText ) =
            case state of
                Pedaler.Disconnected ->
...
subscriptions _ =
    Pedaler.subscriptions PedalerMsg
...
### JavaScript Bridge for Elm

```typescript
// src/elm-pedaler-bridge.ts
import init, { WasmPedalSim } from 'pedaler_core';

interface ElmApp {
  ports: {
    pedalerConnect: { subscribe: (callback: (config: Config) => void) => void };
    pedalerDisconnect: { subscribe: (callback: () => void) => void };
    pedalerUpdateCircuit: { subscribe: (callback: (circuit: string) => void) => void };
    pedalerStateChanged: { send: (state: StateMessage) => void };
    pedalerError: { send: (error: string) => void };
  };
}
...
class ElmPedalerBridge {
  ...
  private setupPorts(): void {
    this.app.ports.pedalerConnect.subscribe(this.handleConnect.bind(this));
    this.app.ports.pedalerDisconnect.subscribe(this.handleDisconnect.bind(this));
    this.app.ports.pedalerUpdateCircuit.subscribe(this.handleUpdateCircuit.bind(this));
  }

  private async handleConnect(config: Config): Promise<void> {
    try {
      this.sendState({ state: 'connecting' });
      ...
      await this.audioContext.audioWorklet.addModule('/pedaler-worklet.js');
      ...
      this.workletNode.port.onmessage = (event) => {
        if (event.data.type === 'ready') {
          this.sendState({ state: 'ready' });
        } else if (event.data.type === 'error') {
          this.sendState({ state: 'error', message: event.data.error });
        }
      };
  ...
  private handleDisconnect(): void {
    ...
    this.sendState({ state: 'disconnected' });
  }

  private handleUpdateCircuit(circuit: string): void {
    if (this.workletNode && this.currentConfig) {
      this.workletNode.port.postMessage({
        type: 'updateCircuit',
        circuit,
      });
  ...
  private sendState(state: StateMessage): void {
    this.app.ports.pedalerStateChanged.send(state);
  }

  private sendError(error: string): void {
    this.app.ports.pedalerError.send(error);
  }
}

// Initialize with Elm app
export function initElmPedalerBridge(app: ElmApp): ElmPedalerBridge {
  return new ElmPedalerBridge(app);
}
```
...
import { initElmPedalerBridge } from './elm-pedaler-bridge';

const app = Elm.Main.init({
  node: document.getElementById('app'),
});

// Connect Pedaler bridge to Elm ports
initElmPedalerBridge(app);
```
...
### Elm Ports Not Working

1. Ensure port names match exactly between Elm and JS
2. Check ports are declared in `port module`
3. Verify Elm app is initialized before setting up bridge
